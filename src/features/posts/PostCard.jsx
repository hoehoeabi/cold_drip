...
export function PostCard({ post, onUpdate }) {
  ...
  const imageUrl = post.images?.[0]
    ? supabase.storage
        ...
        .getPublicUrl(post.images[0].file_path).data.publicUrl
    : null;

  const authorAvatarUrl = post.profiles?.avatar_url
    ? supabase.storage.from("avatars").getPublicUrl(post.profiles.avatar_url)
        .data.publicUrl
    : null;
  ...
  return (
    <li className="flex flex-col bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[32px] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] hover:shadow-[0_20px_50px_rgba(79,70,229,0.1)] transition-all duration-500 group relative">
      {/* 1. 작성 날짜 (우측 상단) */}
      <div className="absolute top-6 right-6 z-10 bg-white/70 dark:bg-slate-950/50 backdrop-blur-md px-3 py-1 rounded-full border border-white/20 dark:border-slate-700/30">
      ...
      <div className="relative overflow-hidden aspect-[4/3]">
        {imageUrl ? (
          ...
          <div className="w-full h-full bg-slate-50 dark:bg-slate-800 grid place-items-center text-slate-300 dark:text-slate-600 font-bold">
            No Image
          </div>
        )}
      ...
      <div className="p-8 flex flex-col flex-1">
        {isEditing ? (
          <div className="grid gap-4">
            <input
              className="w-full p-3 bg-slate-50 dark:bg-slate-800 dark:text-white border-none rounded-xl font-bold"
              value={editTitle}
              onChange={(e) => setEditTitle(e.target.value)}
            />
            <textarea
              className="w-full p-3 bg-slate-50 dark:bg-slate-800 dark:text-white border-none rounded-xl text-sm"
              value={editContent}
          ...
          <>
              ...
              {user?.id === post.author_id && (
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="p-2 hover:bg-amber-50 dark:hover:bg-amber-950/30 text-amber-500 rounded-lg transition-colors"
                  >
                    ✎
                  </button>
                  <button
            ...
            {/* 작성자 프로필: Link 태그가 확실히 동작하도록 구조를 강화했습니다. */}
            <Link
